    ...
    def output(self):
        ...
        if self.number < 3 or self.number > 5:
            ...
            pygame.draw.rect(screen, pygame.Color('white'), (110 + 150 * (self.number % 3),
                                                             260 + 192 * (self.number // 3), 40, 35), 3)
            if self.number < 3:
                name = b_font3.render(f'Перевал {self.number + 4}', 1, pygame.Color('white'))
            else:
                name = b_font3.render(f'Перевал {self.number - 5}', 1, pygame.Color('white'))
            ...
            for i in range(2):
                name = b_font1.render(f'Перевал Хорана'.split()[i], 1, pygame.Color('white'))
                name_coord = name.get_rect()
                name_coord.center = b_battlefields[self.number].center
            ...
            name = b_font3.render(f'Мост {self.number % 3 // 2 + 1}', 1, pygame.Color('white'))
            name_coord = name.get_rect()
            name_coord.center = b_battlefields[self.number].center
            screen.blit(name, name_coord)
